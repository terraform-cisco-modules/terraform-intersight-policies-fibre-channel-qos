...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-fc-qos-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Fibre-Channel QoS Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/FcQosPolicies/%s", moid), expectedJSONTemplate, vars)
}
