...
variable "burst" {
  default     = 1024
  description = "Only Change for FIAttached Server Policy.  The burst traffic, in bytes, allowed on the vHBA.  Value can be between 1024-1000000."
  type        = number
}

variable "cos" {
  default     = 3
  description = "Only Change for Standalone Server Policy.  Class of Service to be associated to the traffic on the virtual interface.  Value can be between 0-6."
  type        = number
}
